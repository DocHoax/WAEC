...
const addSignatures = async (doc, y, studentClass) => {
  ...
  if (signature && signature.teacherSignature && fs.existsSync(path.join(__dirname, '../../Uploads', signature.teacherSignature))) {
    doc.image(path.join(__dirname, '../../Uploads', signature.teacherSignature), col1 + 80, teacherY - 10, { width: 150, height: 30 });
  } else {
    doc.moveTo(col1 + 80, teacherY + 10)
  ...
  if (signature && signature.principalSignature && fs.existsSync(path.join(__dirname, '../../Uploads', signature.principalSignature))) {
    doc.image(path.join(__dirname, '../../Uploads', signature.principalSignature), col1 + 80, principalY - 10, { width: 150, height: 30 });
  } else {
    doc.moveTo(col1 + 80, principalY + 10)
...
router.get('/export/report/:studentId/:sessionName', auth, async (req, res) => {
  try {
    ...
    // Normalize session to match database format (e.g., '2025/2026 First Term')
    const normalizedSession = decodeURIComponent(session)
      .replace(/[-:]/g, '/')  // Replace hyphens or colons with slashes
      .replace(/\s+/g, ' ')   // Normalize spaces
      .trim();

    console.log('ReportCard - Normalized session:', { normalizedSession });
    ...
    if (!sessionExists) {
      console.log('ReportCard - Session not found:', { session: normalizedSession });
    ...
    console.log('ReportCard - Generated successfully:', { 
      studentId, 
      session: normalizedSession, 
    ...
    console.error('ReportCard - Error:', { 
      message: error.message, 
      stack: error.stack, 
      studentId: req.params.studentId, 
      session: req.params.session,
      normalizedSession: decodeURIComponent(req.params.session || '')
        .replace(/[-:]/g, '/')
        .replace(/\s+/g, ' ')
        .trim(),
      timestamp: new Date().toISOString() 
